...
describe("Version Management", () => {
	...
	describe("Version in URLs and Paths", () => {
		test("constructs npm package URL with version", () => {
			const packageName = "@bun-security-scanner/npm";
			const version = "v1.0.1";
			const url = `https://www.npmjs.com/package/${packageName}/v/${version}`;

			expect(url).toBe(
				"https://www.npmjs.com/package/@bun-security-scanner/npm/v/v1.0.1",
			);
		});

		test("constructs GitHub release URL with version", () => {
			const repo = "bun-security-scanner/npm";
			const version = "v1.0.1";
			const url = `https://github.com/${repo}/releases/tag/${version}`;

			expect(url).toBe(
				"https://github.com/bun-security-scanner/npm/releases/tag/v1.0.1",
			);
		});
		...
		test("constructs GitHub blob URL with version", () => {
			...
			expect(url).toBe(
				"https://github.com/bun-security-scanner/npm/blob/v1.0.1/CHANGELOG.md",
			);
		});
	});
	...
	describe("Version Formatting", () => {
		test("formats version for different contexts", () => {
			...
			const npmUrl = `https://www.npmjs.com/package/@bun-security-scanner/npm/v/v${version}`;
