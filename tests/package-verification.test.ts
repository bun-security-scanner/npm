...
npm notice 📦  @bun-security-scanner/npm@1.0.1
...
npm notice name:          @bun-security-scanner/npm
npm notice version:       1.0.1
npm notice filename:      bun-security-scanner-npm-1.0.1.tgz
...
npm notice 2.3kB src/index.ts`;
		...
		test("parses JSON output from npm pack", () => {
			const npmPackJsonOutput = JSON.stringify([
				{
					name: "bun-npm-scanner",
					version: "1.0.1",
					size: 5242880, // 5MB
					unpackedSize: 10485760,
					filename: "bun-npm-scanner-1.0.1.tgz",
				},
			]);

			const parsed = JSON.parse(npmPackJsonOutput);
			expect(parsed[0].size).toBe(5242880);
			expect(parsed[0].name).toBe("bun-npm-scanner");
		});
		...
		test("validates package structure from JSON output", () => {
			...
			expect(packageData.name).toBe("bun-npm-scanner");
	...
	describe("package.json Validation", () => {
		...
		test("validates package.json name format", () => {
			const validNames = ["bun-npm-scanner", "@scope/package", "package-name"];
	...
	describe("Real-World Package Scenarios", () => {
		...
		test("validates actual bun-npm-scanner package structure", () => {
			const expectedFiles = [
				"src/index.ts",
				"src/client.ts",
				"src/processor.ts",
