...
The Bun Audit Scanner is a security-critical component that protects developers from installing vulnerable npm packages. This document outlines our security practices, threat model, and vulnerability reporting procedures.
...
The Bun Audit Scanner operates in a security-sensitive environment and faces several potential threats:

#### **1. Supply Chain Attacks**
- **Threat**: Malicious actors compromising npm registry API responses
- **Mitigation**:
  - HTTPS-only communication with registry.npmjs.org
  - Response validation using Zod schemas
  - Fail-safe behavior on malformed responses

#### **2. API Manipulation Attacks**
- **Threat**: Man-in-the-middle attacks on npm registry API calls
- **Mitigation**:
  - Certificate pinning through HTTPS
  - Request/response integrity validation with gzip compression
...
- **Mitigation**:
  ...
  - Bulk query optimization to reduce API calls
...
```typescript
    ...
    logger.error("npm audit scanner encountered an unexpected error", { error: message });
...
- No external network access beyond npm registry API

#### **4. Transparency**
- All vulnerability data sourced from npm's GitHub Advisory Database
...
```typescript
// npm audit API responses validated against strict schemas
const parsed = NpmAuditResponseSchema.parse(data);
const advisory = NpmAdvisorySchema.parse(data);
```

### **Network Security**
- **HTTPS Enforcement**: All npm registry API calls use HTTPS
- **Request Timeouts**: 30-second default timeout prevents hanging
- **Retry Logic**: Exponential backoff with smart retry conditions
- **Gzip Compression**: Native compression for optimized API performance
- **User Agent**: Identifies scanner for npm registry monitoring
...
NPM_SCANNER_LOG_LEVEL=info        # Controls information disclosure in logs

# Network security
NPM_SCANNER_TIMEOUT_MS=30000      # Prevents long-running requests
NPM_SCANNER_REGISTRY_URL=...      # Allows custom npm registry (enterprise)
```
...
- Bulk query optimization (reduces API surface)
...
#### **1. npm Registry API Trust**
- **Risk**: Scanner depends on npm registry infrastructure integrity
- **Mitigation**: npm registry is Microsoft/GitHub-operated, industry-standard infrastructure
...
- **Enterprise**: Custom NPM_SCANNER_REGISTRY_URL for internal registries

#### **3. Rate Limiting**
- **Risk**: npm registry may rate-limit high-volume usage
- **Mitigation**: Exponential backoff, bulk queries, reasonable timeouts
...
https://github.com/bun-security-scanner/npm/security/advisories/new
...
- Issues in npm registry infrastructure (report to npm)
...
NPM_SCANNER_LOG_LEVEL=warn

# Configure appropriate timeouts for your network
NPM_SCANNER_TIMEOUT_MS=30000

# Use HTTPS-only custom endpoints (enterprise)
NPM_SCANNER_REGISTRY_URL=https://your-internal-registry.company.com
```

#### **2. Network Security**
- Ensure HTTPS connectivity to registry.npmjs.org
- Configure corporate firewalls to allow npm registry access
...
NPM_SCANNER_LOG_LEVEL=debug

# Use shorter timeouts for faster feedback
NPM_SCANNER_TIMEOUT_MS=10000
```
...
NPM_SCANNER_REGISTRY_URL=https://invalid.example.com bun test
```
...
- **Security Vulnerabilities**: https://github.com/bun-security-scanner/npm/security/advisories/new
- **General Issues**: https://github.com/bun-security-scanner/npm/issues
- **Security Advisories**: https://github.com/bun-security-scanner/npm/security
